...
static long long findComp(long long x);
static long long findVir (long long x);
...
int main(){
    ...
    for(int i=1;i<=N;i++){
        ...
        cSize[i]=1; 
        cDomVirus[i]=i;
        netHead[i]=netTail[i]=i; 
        nxtNet[i]=prvNet[i]=NIL;

        vPar[i]=i; 
        vLvl[i]=1; 
        vCnt[i]=1; 
        vTag[i]=0;
        virHead[i]=virTail[i]=i; 
        nxtVir[i]=prvVir[i]=NIL;
...
static long long findComp(long long x){ 
    while(cPar[x]!=x) x=cPar[x]; return x; 
}

static long long findVir (long long x){ 
    while(vPar[x]!=x) x=vPar[x]; return x; 
}
...
static void opConnect(int x,int y){
    ...
    if(cSize[r1] < cSize[r2]){ 
        long long t=r1; r1=r2; r2=t; 
    }
    MOD(&cPar[r2],r1);
    ...
    for(long long u=netHead[r1]; u!=NIL; u=nxtNet[u]){
        ...
        if(old==win){
            if (prvVir[u]==NIL && virHead[win]!=u && curVir[u]==win){
                detachVir(u, win);     
                appendVir(win, u);     
            }
            continue;
...
static void opEvolve(int t){ 
    long long r=findVir(t); 
    MOD(&vLvl[r],vLvl[r]+1); 
}

static void opAttack(int t){ 
    long long r=findVir(t); 
    MOD(&vTag[r],vTag[r]+vLvl[r]); 
}
