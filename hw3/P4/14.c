...
void undoLastOp();
void undoOps(int k);
void assignp(long long x, long long newParent);
Pair findv(long long x);
Pair findc(long long x);
long long calAttack(long long v);
long long unionv(long long x, long long y);
long long unionc(long long x, long long y);
void reset(long long x);
void deletev(long long x);
void evolve(long long v);
void attack(long long v);
void reinstall(long long x, long long v);
void status(long long x);
void setup(long long i);
...
long long unionc(long long x, long long y) {
    ...
    long long v;
    if (v_level[findv(vx).p] >= v_level[findv(vy).p]) {
        v = vx;
    } else {
        v = vy;
    }

    long long ax = calAttack(vx);
    long long ay = calAttack(vy);
    ...
    if (cleft != 0) {
        modify(&c_rightSib[cleft],cright);
    }
    if (cright != 0) {
        modify(&c_leftSib[cright], cleft);
    }
    if (vc[cv] == c) {
        modify(&vc[cv], cright);
    }
