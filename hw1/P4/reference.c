...
int binary_search_for_op2(ll p) {
    int lo = 0, hi = size - 1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        ll cur_val = get_current_value(mid);
        ...
        if (cur_val >= p && last_val <= p)
            return mid;  // Found node containing p
        else if (cur_val < p)
            hi = mid - 1;  // Search higher nodes (larger values)
        else
            lo = mid + 1;  // Search lower nodes (smaller values)
    }
    return -1;  
}
...
void op2(ll p) {
    ...
    // First binary search to find candidate node with cur_val >= p
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        ll cur_val = get_current_value(mid);
        if (cur_val == p) {
    ...
    // Explicitly check adjacent nodes (mid - 1, mid, mid + 1)
